...
export const SignupEmail = ({ siteName, siteUrl, recipient, confirmationUrl }: Props) => (
  <Html lang="es" dir="ltr">
    <Head />
    <Preview>Confirma tu email para activar tu cuenta de Glowapp</Preview>
    <Body style={main}>
      <Container style={container}>
        ...
        <Section style={card}>
          <Text style={badge}>Confirma tu email</Text>
          <Heading style={h1}>¡Ya casi estás!</Heading>
          <Text style={text}>
            Gracias por unirte a <strong style={brand}>Glowapp</strong>. Solo te falta un paso: confirma
            que <Link href={`mailto:${recipient}`} style={link}>{recipient}</Link> es tu email.
          </Text>
          <Button style={button} href={confirmationUrl}>Confirmar mi cuenta</Button>
          <Text style={muted}>Este enlace expira en 24 horas.</Text>
          <Text style={footer}>
...
const muted = { fontSize: '13px', color: '#676B7E', margin: '20px 0 0' }
