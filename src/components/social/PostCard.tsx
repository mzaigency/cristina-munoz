...
export function PostCard({
  ...
  return <div className="bg-card border-b border-border">
      {/* Header */}
      {showHeader && <div className="flex items-center justify-between p-3">
          <button onClick={goToSalon} className="flex items-center gap-3 active:opacity-70 transition-opacity">
      ...
      {/* Image */}
      <div className="relative aspect-square bg-muted" onDoubleClick={handleDoubleTap}>
        <img src={post.image_url} alt={post.caption || "Post"} className="w-full h-full object-cover" loading="lazy" />
        
        {/* Double tap heart animation */}
        <AnimatePresence>
          {showHeart && <motion.div initial={{
          scale: 0,
          opacity: 0
        }} animate={{
          scale: 1,
      ...
      <div className="p-3">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-4">
            ...
            scale: 0.8
          }} onClick={handleLikeClick} className="active:opacity-70">
              <Heart className={cn("w-6 h-6 transition-colors", isLiked ? "fill-red-500 text-red-500" : "text-foreground")} />
            </motion.button>
            <button className="active:opacity-70" onClick={() => {
            haptic.light();
        ...
        {/* Caption */}
        {post.caption && <p className="text-sm">
            <span className="font-semibold mr-1.5">{post.tenant_name}</span>
            {post.caption}
          </p>}

        {/* View comments */}
        {(post.comments_count || 0) > 0 && <button onClick={() => setShowComments(true)} className="text-sm text-muted-foreground mt-1">
