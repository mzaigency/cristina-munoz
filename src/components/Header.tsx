...
export const Header = ({ onNavigate, activeSection }: HeaderProps) => {
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-[100] w-full border-b transition-all duration-300 pt-[env(safe-area-inset-top)] ${isScrolled ? "bg-[#3b2b30] text-white border-[#3b2b30] shadow-lg" : "bg-transparent border-transparent"}`}
    >
              ...
              {navItems.map((item, index) => (
                <div key={item.id}>
                  <DropdownMenuItem
                    ...
                    className={`cursor-pointer ${activeSection === item.id ? "bg-[hsl(var(--salon-pink-light))] text-[hsl(var(--salon-pink))] font-medium" : "hover:bg-[hsl(var(--salon-pink-light))]/50"}`}
                  >
                    {item.label}
                  </DropdownMenuItem>
