...
import { 
  X, Type, Sparkles, Trash2, ChevronRight, Send, Palette, 
  AlignLeft, AlignCenter, AlignRight, Check, Undo2, ChevronLeft,
...
const getAngle = (p1: React.Touch, p2: React.Touch) => (Math.atan2(p2.clientY - p1.clientY, p2.clientX - p1.clientX) * 180) / Math.PI;
    ...
    <AnimatePresence>
      ...
      >
        ...
        <div className="absolute top-0 w-full z-50 flex justify-between items-center p-4 pt-safe-top bg-gradient-to-b from-black/80 to-transparent pointer-events-none">
          {step === "capture" ? (
             <button onClick={onClose} className="pointer-events-auto p-3 glass-ios rounded-full active:scale-90 transition-transform">
               <X size={24} />
             </button>
          ) : (
            <button onClick={handleUndo} className="pointer-events-auto p-3 glass-ios rounded-full active:scale-90 transition-transform">
              <ChevronLeft size={24} />
            </button>
          )}

          {step === "edit" && !isDragging && (
             <button onClick={() => setStep("publish")} className="pointer-events-auto px-6 py-2 bg-white text-black rounded-full font-bold shadow-xl active:scale-95 transition-transform flex items-center gap-2">
        ...
        <div className="flex-1 relative flex items-center justify-center bg-zinc-950">
                 ...
                 <label className="flex flex-col items-center gap-3 cursor-pointer group">
                   ...
                   <input type="file" accept="image/*" capture="environment" onChange={(e) => {
                       ...
                       if(file) {
                           ...
                           reader.onload = (ev) => { setImageData(ev.target?.result as string); setStep("edit"); };
              ...
              {overlays.map((item) => (
                ...
                >
                  ...
                  <div className={cn(
                    ...
                    isDragging && selectedId === item.id && item.y > 0.8 ? "scale-50 opacity-50 grayscale transition-all duration-300" : ""
                  )}>
                    {item.type === "text" ? (
                       item.isEditing ? (
                         <textarea
                           autoFocus
                           value={item.content}
                           onChange={(e) => setOverlays(prev => prev.map(o => o.id === item.id ? {...o, content: e.target.value} : o))}
                           onBlur={() => setOverlays(prev => prev.map(o => o.id === item.id ? {...o, isEditing: false} : o))}
                           className="bg-transparent outline-none resize-none overflow-hidden text-center min-w-[50px]"
                           placeholder="Escribe..."
                           style={{ 
                             fontFamily: item.fontFamily, color: item.color, fontSize: "40px", 
                             textAlign: item.align, textShadow: "0 2px 10px rgba(0,0,0,0.5)" 
                           }}
                           // Auto-height trick
                         ...
                         <div 
                           onClick={(e) => { e.stopPropagation(); setOverlays(prev => prev.map(o => o.id === item.id ? {...o, isEditing: true} : o)); }}
          ...
          {step === "publish" && (
             <div className="w-full max-w-md px-8">
               ...
               <button onClick={handlePublish} disabled={isUploading} className="w-full py-4 bg-white text-black rounded-2xl font-bold text-lg flex items-center justify-center gap-2 disabled:opacity-50">
        ...
        {step === "edit" && !isDragging && !overlays.some(o => o.isEditing) && (
          <motion.div initial={{ x: -100 }} animate={{ x: 0 }} className="absolute left-4 top-1/2 -translate-y-1/2 flex flex-col gap-6 z-40">
            <button onClick={(e) => { e.stopPropagation(); handleCanvasClick(e as any); }} className="w-12 h-12 rounded-full glass-ios flex items-center justify-center shadow-lg active:scale-90 transition-transform">
              <Type size={24} />
            </button>
            <button onClick={(e) => { e.stopPropagation(); setShowTools("stickers"); setSelectedId(null); }} className="w-12 h-12 rounded-full glass-ios flex items-center justify-center shadow-lg active:scale-90 transition-transform">
              <Sparkles size={24} />
            </button>
            <button onClick={(e) => { e.stopPropagation(); setShowTools("filters"); setSelectedId(null); }} className="w-12 h-12 rounded-full glass-ios flex items-center justify-center shadow-lg active:scale-90 transition-transform">
              <Palette size={24} />
            </button>
          </motion.div>
        )}
        ...
        <AnimatePresence>
          {showTools !== "none" && (
            ...
            >
              <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={() => setShowTools("none")} className="p-1 bg-white/10 rounded-full"><X size={16} /></button>
              </div>
              ...
              {showTools === "filters" && (
                <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide">
                  {FILTERS.map(f => (
                    <button key={f.id} onClick={() => setActiveFilter(f.id)} className="flex flex-col items-center gap-2 group">
                      <div className={cn("w-20 h-24 rounded-lg overflow-hidden border-2 transition-all", activeFilter === f.id ? "border-white" : "border-transparent opacity-60")}>
              ...
              {showTools === "stickers" && (
                <div className="grid grid-cols-5 gap-4">
                  {STICKERS.map((s, i) => (
                    <button key={i} onClick={() => addSticker(s)} className="text-4xl hover:scale-125 transition-transform active:scale-90">{s}</button>
                  ))}
                </div>
              )}
            </motion.div>
          ...
          {overlays.some(o => o.isEditing) && (
             <motion.div initial={{ y: 100 }} animate={{ y: 0 }} exit={{ y: 100 }} className="absolute bottom-0 left-0 right-0 glass-ios p-4 z-[150] flex flex-col gap-4 pb-safe border-t border-white/10">
                <div className="flex gap-3 overflow-x-auto scrollbar-hide justify-center">
                   {FONTS.map(f => (
                     <button key={f.id} onClick={() => { 
                         ...
                         setCurrentFont(f.id);
                     }} className={cn("px-4 py-1.5 rounded-full text-sm font-bold border transition-all whitespace-nowrap", (overlays.find(o => o.isEditing)?.fontFamily === f.family) ? "bg-white text-black border-white" : "border-white/20 text-white")}>
                       {f.name}
                     </button>
                   ))}
                </div>
                <div className="flex items-center justify-between px-2">
                   <div className="flex gap-1 bg-white/10 rounded-lg p-1">
                      {[{i: <AlignLeft size={16}/>, v:'left'}, {i: <AlignCenter size={16}/>, v:'center'}, {i: <AlignRight size={16}/>, v:'right'}].map(b => (
                        <button key={b.v} onClick={() => {
                          const id = overlays.find(o => o.isEditing)?.id;
                          if(id) updateOverlay(id, { align: b.v as any });
                        }} className={cn("p-2 rounded", overlays.find(o => o.isEditing)?.align === b.v ? "bg-white text-black" : "text-white")}>{b.i}</button>
                      ))}
                   </div>
                   <div className="flex gap-2 overflow-x-auto max-w-[180px] scrollbar-hide px-2">
                      {COLORS.map(c => (
                        <button key={c} onClick={() => {
                           ...
                           setCurrentColor(c);
                        }} className={cn("w-8 h-8 rounded-full border-2 shrink-0 transition-transform", (overlays.find(o => o.isEditing)?.color === c) ? "scale-110 border-white" : "border-transparent")} style={{backgroundColor: c}} />
                      ))}
                   </div>
                   <button onClick={() => {
                      const id = overlays.find(o => o.isEditing)?.id;
                      if(id) updateOverlay(id, { isEditing: false });
                   }} className="w-10 h-10 rounded-full bg-white text-black flex items-center justify-center active:scale-90"><Check size={20} strokeWidth={3} /></button>
                </div>
             </motion.div>
          )}
        </AnimatePresence>
