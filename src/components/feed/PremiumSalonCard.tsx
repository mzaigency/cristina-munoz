      ...
      <Link to={`/salon/${salon.slug}`} className="block">
        <div className="relative overflow-hidden rounded-[28px] bg-card border border-border/30 shadow-xl shadow-foreground/[0.03] transition-all duration-500 group-hover:shadow-2xl group-hover:shadow-primary/15 group-hover:border-primary/30">
          ...
          <div className="relative h-52 overflow-hidden">
            {salon.hero_image_url ? <motion.img src={salon.hero_image_url} alt={salon.name} className="w-full h-full object-cover" initial={{
            scale: 1.1
          }} whileHover={{
            scale: 1.15
          }} transition={{
            ...
            background: `linear-gradient(135deg, ${primaryColor}ee, ${primaryColor}88)`
                ...
                {salon.logo_url ? <img src={salon.logo_url} alt={salon.name} className="h-24 w-24 object-contain rounded-3xl bg-white/95 p-4 shadow-2xl" /> : <div className="h-24 w-24 rounded-3xl bg-white/15 backdrop-blur-2xl flex items-center justify-center text-white text-3xl font-bold shadow-2xl border border-white/20">
                    {initials}
                  </div>}
              </div>}
            ...
            <div className="absolute top-3.5 left-3.5 flex flex-wrap gap-2">
              {hasHighRecommendation && !hasAvailabilityToday && !isPopular && !isNew && <RecommendationBadge score={recommendationScore!} compact />}
              {hasAvailabilityToday && <motion.div initial={{
              opacity: 0,
              x: -12,
              scale: 0.9
            }} animate={{
            ...
            scale: 0.8
          }} onClick={handleFavoriteClick} className={cn("absolute top-3.5 right-3.5 h-11 w-11 rounded-full flex items-center justify-center transition-all duration-400", isFav ? "bg-rose-500 text-white shadow-xl shadow-rose-500/40" : "bg-black/20 backdrop-blur-xl text-white hover:bg-black/30 border border-white/10")}>
              <motion.div animate={{
            ...
            {salon.avgRating !== null && <motion.div initial={{
            opacity: 0,
            y: 10
          }} animate={{
            opacity: 1,
          ...
          <div className="p-5">
            <div className="flex items-start justify-between gap-3 mb-2">
              ...
              {salon.average_price && <span className="shrink-0 text-xs font-bold text-primary bg-primary/10 px-2.5 py-1 rounded-lg">
                  Desde {salon.average_price}€
                </span>}
            </div>
            
            {salon.tagline && <p className="text-sm text-muted-foreground mb-3 line-clamp-2 leading-relaxed">
                {salon.tagline}
              </p>}

            {/* Match reasons for recommendations */}
            {matchReasons && matchReasons.length > 0 && <div className="flex flex-wrap gap-1 mb-3">
                {matchReasons.slice(0, 2).map((reason, i) => <span key={i} className="text-[10px] text-primary bg-primary/10 px-2 py-0.5 rounded-full font-medium">
                    {reason}
                  </span>)}
              </div>}
            ...
            <div className="flex flex-col gap-3 pt-3 border-t border-border/40">
              ...
              <div className="flex items-center gap-3">
                {distance && <div className="flex items-center gap-1.5 text-primary">
                    <Navigation className="h-3.5 w-3.5" />
                    <span className="text-sm font-bold">{distance}</span>
                  </div>}
                {salon.city && <div className="flex items-center gap-1.5 text-muted-foreground">
              ...
              <div className="flex items-center gap-2">
                ...
                scale: 1.02
              }} onClick={handleFollowClick} disabled={followLoading} className={cn("flex items-center justify-center gap-1.5 h-9 px-4 rounded-full text-xs font-semibold transition-all flex-1", following ? "bg-primary/10 text-primary border border-primary/30" : "bg-secondary text-secondary-foreground hover:bg-secondary/80 border border-border/50")}>
                  {following ? <>
