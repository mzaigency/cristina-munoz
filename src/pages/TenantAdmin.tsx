...
export default function TenantAdmin() {
  ...
  const navItems: NavItem[] = useMemo(() => {
    ...
    // If user is stylist but not admin, hide Settings, Team and Stats tabs
    if (isStylist && !isAdmin) {
      return allItems.filter((item) => item.value !== "settings" && item.value !== "team" && item.value !== "stats");
    }
  ...
  const renderContent = () => {
    ...
    switch (activeTab) {
      ...
      case "agenda":
        return <AgendaSection tenantId={tenant.id} />;
      case "clients":
        return <ClientsCRM tenantId={tenant.id} />;
      case "business":
        return <BusinessSection tenantId={tenant.id} />;
      case "stats":
        return <StatsSection tenantId={tenant.id} />;
      case "team":
        return <TeamSection tenantId={tenant.id} />;
      case "communication":
        return <CommunicationSection tenantId={tenant.id} />;
      case "settings":
        return <SettingsSection tenantId={tenant.id} tenantSlug={tenant.slug} />;
      default:
        return null;
    }
  };
  ...
  return (
    <div className="min-h-screen bg-muted/30">
      ...
      {/* Content with Swipe navigation on mobile */}
      {isMobile ? (
